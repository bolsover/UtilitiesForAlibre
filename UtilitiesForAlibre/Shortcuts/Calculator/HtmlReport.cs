    ...
    public class HtmlReport
    {
        ...
        public string BuildReport(string profile)
        {
            ...
            if (userShortcuts.Count > 0)
            {
                ...
                var standardShortcutsDict = calculator.ShortcutsDictionary(standardShortcuts);
                foreach (var sc in userShortcuts)
                {
                    if (standardShortcutsDict.TryGetValue(sc.Profile + "." + sc.Command, out var standardShortcut))
                    {
                        sc.ShortcutType = standardShortcut.KeyChar == sc.KeyChar
                            ? ShortcutType.Default
                            : ShortcutType.Override;
                    }
                    else
                    {
        ...
        private string BuildHtml(List<AlibreShortcut> shortcuts, string[] titles, int subTables, bool isStandard,
            string profile)
        {
        ...
        private string BuildTableData(List<AlibreShortcut> shortcuts, string[] titles, int subTables)
        {
            ...
            if (j < shortcuts.Count)
            {
                sb.Append(BuildTableHeader());
                sb.Append(BuildTableTitles(titles));
                for (var i = j; i < shortcuts.Count; i++)
                {
        ...
        {
            StringBuilder sb = new();
            var color = alibreShortcut.ShortcutType switch
            {
            ...
            sb.Append($"<tr {color}>");
            var imgSrc = "data:image/png;base64," + SvgToPng(alibreShortcut.SvgImage);
        ...
        private string BuildBodyHeader(string profile, bool isStandard)
        {
            ...
            var texta = "<p><span style = \"color:black\">Black - Standard Shortcut " +
