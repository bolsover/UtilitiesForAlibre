...
namespace Bolsover.Involute.Model
{
    ...
    public class Trochoid
    {
        ...
        public double HypoTrochoidX(double theta)
        {
            return HypoModulus * Math.Cos(Radians(theta)) +
                   TracePoint * Math.Cos(HypoModulus / RollingCircleRadius *
                                         Radians(theta));
        }
        
        public double HypoTrochoidY(double theta)
        {
            return HypoModulus * Math.Sin(Radians(theta)) -
                   TracePoint * Math.Sin(HypoModulus / RollingCircleRadius *
                                         Radians(theta));
        }
        
        
        public double EpiTrochoidX(double theta)
        {
            return EpiModulus * Math.Cos(Radians(theta)) -
                   TracePoint * Math.Cos(
                       EpiModulus / RollingCircleRadius * Radians(theta));
        }
        
        public double EpiTrochoidY(double theta)
        {
            return EpiModulus * Math.Sin(Radians(theta)) -
                   TracePoint * Math.Sin(
                       EpiModulus / RollingCircleRadius * Radians(theta));
        }
        ...
        public static double PolarRinv(double baseDiameter, double zeta)
        {
            return baseDiameter / 2 * Math.Sqrt(1 + Math.Pow(zeta, 2));
        }
        
        public static double PolarEtaInv(double zeta)
        {
            return zeta - Math.Atan(zeta);
        }
        ...
        {
            return Math.Sqrt(Math.Pow(baseDiameter / 2, 2) + Math.Pow(delta, 2) -
                             baseDiameter * delta * Math.Cos(Math.PI / 2 - lambda));
        }
        
        public static double PolarEtaTro(double theta, double epsilon, double alpha)
        {
            return theta + epsilon + alpha;
        }
        
        public double TanEpsilon(double baseDiameter, double lambda, double delta)
        {
            return delta * Math.Cos(lambda) / (baseDiameter / 2 - delta * Math.Sin(lambda));
        }
    }
}